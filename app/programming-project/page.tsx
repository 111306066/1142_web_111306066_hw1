import Image from "next/image";
import Link from "next/link";

const programmingProjects = [
  {
    title: "supportYOU",
    description:
      "透過訓練大型語言模型建立一個提供專業心理支持的聊天機器人",
    image: "/code-1.png",
    href: "https://drive.google.com/file/d/14jgFCFjnD00lb4VQoTHj6auGwCGlTvZi/view?usp=sharing",
  },
  {
    title: "安行寶",
    description:
      "設計一款讓長者出行時更安全的APP",
    image: "/code-2.png",
    href: "https://canva.link/guil5x6p4ob3vxj",
  },
];

export default function ProgrammingProjectPage() {
  return (
    <section className="page-section">
      <h1 className="section-title">程式專案</h1>
      <p className="section-text project-page-intro">
        這裡整理我做過的網站、互動作品與系統開發練習。
      </p>
      <div className="project-grid">
        {programmingProjects.map((project) => (
          <article key={project.title} className="project-card">
            <div className="project-image-wrap">
              <Image
                src={project.image}
                alt={project.title}
                fill
                className="project-image"
              />
            </div>

            <div className="project-card-body">
              <h2 className="project-card-title">{project.title}</h2>
              <p className="project-card-description">{project.description}</p>

              <Link href={project.href} className="project-card-button">
                查看更多
              </Link>
            </div>
          </article>
        ))}
      </div>
    </section>
  );
}